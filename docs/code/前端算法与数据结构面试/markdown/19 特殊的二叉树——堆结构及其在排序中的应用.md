...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b51adec191fc~tplv-t2oaga2asx-watermark.awebp) 也可以是这样的：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b53f668d2a89~tplv-t2oaga2asx-watermark.awebp)

但不能是这样的：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b5699b3d67bb~tplv-t2oaga2asx-watermark.awebp)

更不能是这样的：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b574e26782a3~tplv-t2oaga2asx-watermark.awebp)

注意，完全二叉树中有着这样的索引规律：假如我们从左到右、从上到下依次对完全二叉树中的结点从0开始进行编码：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b59096075d9e~tplv-t2oaga2asx-watermark.awebp) 那么对于索引为 `n` 的结点来说：
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b5a74a28f22e~tplv-t2oaga2asx-watermark.awebp) 若树中每个结点值都不大于其左右孩子的结点值，这样的完全二叉树就叫做“小顶堆”

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b5b1c6f65e49~tplv-t2oaga2asx-watermark.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b5bc43da2b69~tplv-t2oaga2asx-watermark.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b600c6dc5536~tplv-t2oaga2asx-watermark.awebp) 交换后，继续向下对比1与当前左右孩子的值，如果其中一个大于1，则交换两者的位置：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b606e67443cb~tplv-t2oaga2asx-watermark.awebp) 重复这个**向下对比+交换**的过程，直到无法继续交换为止，我们就得到了一个符合“大顶”原则的新的堆结构：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b612b4981b7c~tplv-t2oaga2asx-watermark.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b62cea61942c~tplv-t2oaga2asx-watermark.awebp) 2. 不断进行**向上对比+交换**的操作：如果发现10比父结点的结点值要大，那么就和父结点的元素相互交换，再接着往上进行比较，直到无法再继续交换为止。首先被比下去的是值为6的直接父结点：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b63d54377138~tplv-t2oaga2asx-watermark.awebp) 接着继续往上找，发现10比根结点9还要大，于是继续进行交换：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b65002f6213a~tplv-t2oaga2asx-watermark.awebp) 根结点被换掉后，再也无法向上比较了。此时，我们已经得到了一个追加过数字10的新的堆结构：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/5/31/1726b65707a61d93~tplv-t2oaga2asx-watermark.awebp)
...
在向下比较+交换的时候，如果是大顶堆，那么当前节点需要和其左右孩子节点中较大的那个比较； 如果是小顶堆，那么当前节点需要和其左右孩子节点中较小的那个比较
...
1、完全二叉树的特例 ==》 堆 ===> 大顶堆、小顶堆 ==》层序遍历 所得数组 ==》 优先队列 ===》 二叉堆结构 2、 可以在不对 序列 进行 完全排序 的情况下，找到第 k 个最值 3、第k个最值 ===》 优先队列（二叉堆数组）==》初始化k个结点的小顶堆 ==》更新小顶堆 ===》返回堆顶元素 4、更换堆顶元素 ==》向下对比与交换 ； 结点入堆 ==》向上对比与交换

1

回复

[![卿落的头像](https://p3-passport.byteacctimg.com/img/user-avatar/99a4acc18681139d65aa5ce5838209ae~300x300.image)](https://juejin.cn/user/2392135676732376)

[卿落](https://juejin.cn/user/2392135676732376)

前端 @ 索贝1年前

修言大大，我想请问第二种使用堆结构的解法，比先排序的解法好在何处呢

点赞

1

[![img](https://p9-passport.byteacctimg.com/img/user-avatar/9b9019437342dcff80d0cad7441dee51~300x300.image)](https://juejin.cn/user/2330620383755854)

[guaiyihezi](https://juejin.cn/user/2330620383755854)

9月前

上面提到过，面试官问排序那个题能不能用堆排序写出来。其实并不能和先排序的性能做比较，只是面试官的任务罢了![👨](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-assets/asset/twemoji/2.6.0/svg/1f468.svg~tplv-t2oaga2asx-image.image)‍![💻](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-assets/asset/twemoji/2.6.0/svg/1f4bb.svg~tplv-t2oaga2asx-image.image)
...
[![得能莫忘的头像](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/3/28/17120be281721249~tplv-t2oaga2asx-no-mark:100:100:100:100.awebp)](https://juejin.cn/user/3491704663184552)

[得能莫忘](https://juejin.cn/user/3491704663184552)

1年前

调用downHeap的时候是不是应该传k-1呀，k-1才是下标的上界
...
啊......广联达今年提前批笔试第一题就是用堆，对前端太不友好了吧，我说怎么java方向的ac率那么高![💔](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-assets/asset/twemoji/2.6.0/svg/1f494.svg~tplv-t2oaga2asx-image.image)

点赞

回复

[![吾乃Jiraiya的头像](https://p3-passport.byteacctimg.com/img/user-avatar/96c4df3c989f9f20c698aff27e09e2fb~300x300.image)](https://juejin.cn/user/1503787636493624)

[吾乃Jiraiya![lv-2](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/f597b88d22ce5370bd94495780459040.svg)](https://juejin.cn/user/1503787636493624)

前端开发 @ 友信金服1年前

我现在终于感受到了学习算法的好处，看react源码的时候发现里面各种队列 链表栈，还有最新单个提出来的包scheduler里面维护的就是一个大顶堆，另外一些生僻难啃的位运算符在各种包里面大量应用，感觉也是贼有用的东西

4

1

[![img](https://p3-passport.byteacctimg.com/img/user-avatar/c2b0b1587eea556ac8e1663c9d8f4e63~300x300.image)](https://juejin.cn/user/483440845920904)

[平平无奇古哥哥](https://juejin.cn/user/483440845920904)

1年前

哈哈，贼有意思，react的scheduler包就是用优先队列，小顶堆polyfill了一个requestIdleCallback，然后Fiber和hooks的设计都是基于各种链表操作。vue里面的keep-alive组件用到了lru缓存算法，虚拟dom树里面用到了深度优先遍历。用数据结构和算法看源码的感觉，特别清亮![😄](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-assets/asset/twemoji/2.6.0/svg/1f604.svg~tplv-t2oaga2asx-image.image)
...
用堆里的最后一个元素（对应图中的数字1）替换掉堆顶元素。 这一步在代码里貌似没有体现

1

1

[![img](https://p26-passport.byteacctimg.com/img/mosaic-legacy/3791/5070639578~300x300.image)](https://juejin.cn/user/764915822104423)
