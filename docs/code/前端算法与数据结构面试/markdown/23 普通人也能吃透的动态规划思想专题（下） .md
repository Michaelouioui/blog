...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b327fe8772890~tplv-t2oaga2asx-watermark.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b32926a1cb7c7~tplv-t2oaga2asx-watermark.awebp) 接下来我尝试用第 `i` 行的数据更新它。当数据更新走到 `dp[i][v]` 这里的时候，`dp[i-1][v]` 和 `dp[i-1][v-w[i]]` 都是存在的状态（分别对应一维数组中现在的 `dp[v]`和`dp[v-w[i]]`的值，完全可以满足我们的计算需要：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b32aef3d64294~tplv-t2oaga2asx-watermark.awebp)

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b32c6f80be822~tplv-t2oaga2asx-watermark.awebp) 当我们计算出 `dp[i][v]` 的值以后，`dp[i-1][v-w[i]]`可能还会在以后的计算中用到，但`dp[i-1][v]`其实已经完全用不到了（这个点对初学的同学来说可能会有点绕，不要慌，你品，你细品。注意这里`dp[i][v]`已经求解出来了，对于 `i` 这个索引来说只需要求解 `dp[i][v-1]`到`dp[i][w[i]]`之间的值，仔细想想，求解这些值是不是完全用不到`dp[i-1][v]`？）。
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b30e1f361fc8c~tplv-t2oaga2asx-watermark.awebp) 同时对于索引位为0的元素来说，由于以它为结尾的子序列有且仅有 [10] 这一个，因此它的状态值时一开始就明确的，那就是1：
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b310c6eeb4358~tplv-t2oaga2asx-watermark.awebp) 发现9比10小，没办法延长原有的子序列，因此啥也不干。继续往下遍历，遇到了2，发现2比前两个数都小，仍然没法延长任何一个子序列，继续啥也不干。

再往下遍历，遇到了5，对比5和前面三个元素，发现它比2大，可以延长2所在的那个最长上升子序列，延长后，以 5 为结尾的最长上升子序列的长度就得到了更新：

![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/14/172b3133fc10d147~tplv-t2oaga2asx-watermark.awebp)
...
[![Ivana007的头像](https://p26-passport.byteacctimg.com/img/mosaic-legacy/3793/3131589739~300x300.image)](https://juejin.cn/user/2066737590441639)

[Ivana007](https://juejin.cn/user/2066737590441639)

web前端工程师1月前

文中最长递增子序列时间复杂度O(n2)了哦
...
背包问题本人卡在这里许久，终于有所领悟，写了篇文章，希望能帮助到跟我一样的人 进死胡同一个月，终于捋清背包问题了：[![img](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/3f843e8626a3844c624fb596dddd9674.svg)juejin.cn](https://juejin.cn/post/7047757948822487048/)
...
这公式里的c一会是容量一会是价值数组的，是我没看懂还是大佬写错了

点赞

1

[![img](https://p6-passport.byteacctimg.com/img/user-avatar/2ade67140f3e272aa2f5a145aa9db644~300x300.image)](https://juejin.cn/user/3825956193316189)

[Junyang1023![lv-1](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/636691cd590f92898cfcda37357472b8.svg)](https://juejin.cn/user/3825956193316189)

1月前

我也卡在这里很久，现在比较清晰，总结了个文章，希望能帮到你[![img](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/3f843e8626a3844c624fb596dddd9674.svg)juejin.cn](https://juejin.cn/post/7047757948822487048/)
...
背包问题不能从 1 开始，会把 w[0] 漏掉。Soulkey评论的knapsack(3, 30, [7, 15, 22], [1, 2, 3]) 这样的入参，确实输出是错误的

点赞

1

[![img](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADwAAAA8AQMAAAAAMksxAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAAA5JREFUKM9jGAWjAAcAAAIcAAE27nY6AAAAAElFTkSuQmCC)](https://juejin.cn/user/4230576475209821)
...
假如knapsack(3, 30, [7, 15, 22], [1, 2, 3])，这样的入参，为啥输出是3呢？选取1和3物品，体积是29也没超过30，最大的价值不应该是1+3=4么，有小伙伴可以试试我这个入参。
...
确实有问题[![img](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/3f843e8626a3844c624fb596dddd9674.svg)juejin.cn](https://juejin.cn/post/7047757948822487048/)

点赞

回复

[![prism的头像](https://p6-passport.byteacctimg.com/img/mosaic-legacy/3795/3033762272~300x300.image)](https://juejin.cn/user/3060685883577741)

[prism](https://juejin.cn/user/3060685883577741)
...
对 背包问题 的一点理解： 1、不一定背包装满才是最大价值，也不一定每个物品装进去才是最大价值， 依据动态规划的 重复子问题 的特性，需要遍历i个物品对应不同容量（v）的最大价值，不断迭代，得到最终结果； 2、外层循环遍历每一个物品，内层循环 遍历背包容量（前i个物品都会对应 c 到 w[i] 的多个决策） 3、物品放进去是 dp[v-w[i]]+value[i]，不放进去是 dp[v]，因为初始化数组元素都是0； 4、第 i 个物品放进去与否由 i-1 时max( dp[v-w[i]]+value[i]，dp[v] ) 决定（转态转移、最优子结构） 5、逆向遍历第i个物品时，i-1的dp[v]只会用到一次，可以用滚动数组(一维数组)，替换不需要的数据（dp[i-1][v]） 6、反之，正向遍历，可能会重复用到dp[i-1][v]，（可用具体理解抽象），所以这样就得用二维数组，提高了空间复杂度；
...
[![左岸诗人的头像](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADwAAAA8AQMAAAAAMksxAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAAA5JREFUKM9jGAWjAAcAAAIcAAE27nY6AAAAAElFTkSuQmCC)](https://juejin.cn/user/3016715636837006)

[左岸诗人](https://juejin.cn/user/3016715636837006)

web前端7月前

很多人对背包问题的索引有疑问，那就不妨按照自己的想法改一改，跑起来也是正确的。
...
[![袁亚雄的头像](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADwAAAA8AQMAAAAAMksxAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAAA5JREFUKM9jGAWjAAcAAAIcAAE27nY6AAAAAElFTkSuQmCC)](https://juejin.cn/user/3808364009627998)

[袁亚雄](https://juejin.cn/user/3808364009627998)
...
[![不甘心的翔入非非的头像](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAADwAAAA8AQMAAAAAMksxAAAAA1BMVEUAAACnej3aAAAAAXRSTlMAQObYZgAAAA5JREFUKM9jGAWjAAcAAAIcAAE27nY6AAAAAElFTkSuQmCC)](https://juejin.cn/user/2471357871034423)

[不甘心的翔入非非](https://juejin.cn/user/2471357871034423)

11月前

const dp = (new Array(c+1)).fill(0) 这里为啥要c+1啊，哪位大佬给解释解释呢？
...
哈哈，因为从1开始，数组的索引是0，他后面的写法是遍历到第n个，数组长度就要n+1
...
因为遍历到的容量是正整数，>=1，没有0，c==100，数组长度就要101
...
画画图，写写方程，通透！![😘](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-assets/asset/twemoji/2.6.0/svg/1f618.svg~tplv-t2oaga2asx-image.image)

![img](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a37c65b6735c43a8a8a426ed8102d132~tplv-k3u1fbpfcp-no-mark:320:320:320:320.awebp)
...
滚动数组这里真不好解释，还是要自己画图或者写写才能明白

![img](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7afe257551e04f01b0bbc07741cef69f~tplv-k3u1fbpfcp-no-mark:320:320:320:320.awebp)
...
龟龟，其实就是两个数组的事情，一个是重量，一个是容量，这就和前面求硬币一样的，只不过在算的过程中有一个重量转换价值的过程，这章确实讲的一般，还是得画图才行，光列方程对于我们这种新手，有点难。

![img](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fb76ac292bce4f33b0e3c0b7c5541df6~tplv-k3u1fbpfcp-no-mark:320:320:320:320.awebp)

1

回复

[![禾小沐的头像](https://p9-passport.byteacctimg.com/img/user-avatar/35bd71e157607ce1c87595859b3a24d9~300x300.image)](https://juejin.cn/user/1732486055606670)

[禾小沐![lv-1](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/636691cd590f92898cfcda37357472b8.svg)](https://juejin.cn/user/1732486055606670)

公众号： 禾小沐的技术与生活11月前

那个背包的,人家问的是怎么放,求个总和有什么用啊?????
...
。。。。这一部分是我觉得唯一讲的没有那么通俗的一章，我在b站发现了一个讲的比较简单易懂视频 [![img](https://lf3-cdn-tos.bytescm.com/obj/static/xitu_juejin_web/3f843e8626a3844c624fb596dddd9674.svg)www.bilibili.com](https://link.juejin.cn/?target=https%3A%2F%2Fwww.bilibili.com%2Fvideo%2FBV18x411V7fm%3Ffrom%3Dsearch%26seid%3D18033740744859035301)

4

1

[![img](https://p6-passport.byteacctimg.com/img/mosaic-legacy/3791/5035712059~300x300.image)](https://juejin.cn/user/3175045313602301)
...
老哥牛逼，考研 55 分数学的一看到数学方程式就头疼。。。看了半个小时还是在云里雾里。

点赞

回复

[![妖猴的头像](https://p26-passport.byteacctimg.com/img/user-avatar/412d38a94f341307a1766b767f1cda85~300x300.image)](https://juejin.cn/user/641770521636679)

[妖猴](https://juejin.cn/user/641770521636679)

1年前

确实难度陡增，说下关键理解：第一个物品遍历到第n个物品，对每一个做决策。动态规划对于背包空间m的所有可能性都做了决策并相应地记录为v[i][j]，所以你看到了一个二重循环，第一层对物品遍历，第二层对于背包容量做遍历。你再仔细理解一下那个递推公式，它的定义是：前i个物品，背包空间为j的情况下所能获得的最大价值。所以装不装得下这个概念，主要是针对背包空间j而言的，而不是只对第i个物品而言的。在对第i个物品做决策的时候，它是需要做m个决策的（j=1..m)，v[i][j]对于每一个j的决策都不一定相同(取或不取物品i)
...
背包的二维数组解法，循环体积那一层循环，是不是要从0开始比较好呢，从w[i]开始会让w[i]之前的值没有记录上
