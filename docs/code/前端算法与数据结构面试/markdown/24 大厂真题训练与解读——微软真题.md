...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/21/172d6684ea1e69d9~tplv-t2oaga2asx-watermark.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/21/172d6697959c2861~tplv-t2oaga2asx-watermark.awebp)
...
![img](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/6/21/172d66a1977ba684~tplv-t2oaga2asx-watermark.awebp)
...
1. 如何处理深拷贝过程中的结点关系：笔者在这里最推荐的一种做法是用 `Map`结构：在这道题中，除了 `next` 指针还有 `random`指针，结点关系相对复杂，这就意味着我们在处理结点关系的过程中必然会遇到“根据原结点定位它对应的copy结点”这样的需求。`Map`结构可以帮我们做到这一点。
2. `next`指针和`random`指针各自应该如何处理：我们可以先走一遍普通链表（也就是没有`random`指针）的复制流程。在这个过程中，一方面是完成对结点的复制+存储工作，另一方面也用`next`指针把新链表串了起来。这一步做完之后，新链表和老链表之间唯一的区别就在于 `random`指针了。此时我们只需要**同步遍历新旧两个链表**，把 `random` 的指向映射到新链表上去即可。
...
[![整只羊的头像](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/20/17195b3f37bad934~tplv-t2oaga2asx-no-mark:100:100:100:100.awebp)](https://juejin.cn/user/4230576475209821)
...
左子树的结点个数numLeft = k - 1，为什么是k-1?假如根结点索引为1即k，那左子树结点个数为1。不应该减1呀

点赞

1

[![img](https://p6-passport.byteacctimg.com/img/user-avatar/e9adbbe62b55e2510ce6d114f78dfc0b~300x300.image)](https://juejin.cn/user/3958685947402717)
...
靠后序、中序还原树： const recoverTree = function(postorder, inorder){ if( !postorder.length || !inorder.length ) return null let root = { val: postorder[postorder.length - 1] } let indexInOrder = inorder.indexOf(postorder[postorder.length - 1]) if( indexInOrder == 0 ) { root.left = null } else { let newInorder = inorder.slice(0, indexInOrder) let newPostorder = postorder.slice(0, newInorder.length) root.left = recoverTree(newPostorder, newInorder) } if( indexInOrder == inorder.length - 1 ) { root.right = null } else { let newInorder = inorder.slice(indexInOrder + 1) let newPostorder = postorder.slice(-newInorder.length - 1, -1) root.right = recoverTree(newPostorder, newInorder) } return root }
...
// 另外作者的方案因为一直是同一个数组，感觉不用用 numLeft，直接 k 就是真实的位置，更直观 function buildTree2(preOrder, inOrder) { let len = preOrder.length function build(preL, preR, inL, inR) { // 重要：要同时判断两个数组 if (preL > preR || inL > inR) { return null } let root = new TreeNode() root.val = preOrder[preL] let k = inOrder.indexOf(root.val) root.left = build(preL+1, k, inL, k - 1) root.right = build(k + 1, preR, k+1, inR) return root } return build(0, len - 1, 0, len - 1) }
...
// 先中序生成二叉树试了个用slice的方式function buildTree(preOrder, inOrder) { let len = preOrder.length if (!len) { return null } let rootVal = preOrder[0] let root = new TreeNode(rootVal) let k = inOrder.indexOf(rootVal) if (k !== 0) { let preLeft = preOrder.slice(1, k+1) let inLeft = inOrder.slice(0, k) root.left = buildTree(preLeft, inLeft) } if (k !== len - 1) { let preRight = preOrder.slice(k+1) let inRight = inOrder.slice(k+1) root.right = buildTree(preRight, inRight) } return root }

1

回复

[![靠近我，温暖你的头像](https://p26-passport.byteacctimg.com/img/user-avatar/20d9152a6d458fd922bdca154ce4c8bf~300x300.image)](https://juejin.cn/user/2823201592453911)

[靠近我，温暖你](https://juejin.cn/user/2823201592453911)
